...
void solve() {
    int n;
    cin >> n;
    // For odd values, print from n to 1 as the middle value stays the same, i.e., the fixed position
    if (n & 1) {
        rloop(i, n, 1) cout << i << ' ';
        cout << '\n';
    }
    else cout << -1 << '\n'; // Permutations with fixed position not possible for even numbered values
}
