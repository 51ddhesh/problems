...
void solve() {
    ...
    // 1 + 2 + 3 + ... + n = n(n + 1)/2
    const int n = 100;
    ll s1 = n * (n + 1) * (2 * n + 1) / 6; // find the sum of squares of natural numbers
    ll s2 = n * (n + 1) / 2; // find the sum of natural numbers
    s2 *= s2; // find the square of sum of natural numbers
    cout << s2 - s1 << '\n';
}
